...
To test out the application, upload the shapefile located at `shapefile/LB_MSOA2021_shp.zip`!


## Challenges
Overall, I'm pretty happy with the results of this project, and I think it went pretty smoothly.
There were a few unexpected challenges though I wanted to call out:

### `react-leaflet` ended up being a bit of a dead end
Initially, I chose [react-leaflet](https://github.com/PaulLeCam/react-leaflet) for rendering the
map - especially for small to medium size datasets where performance isn't a massive consideration,
a tool like this means as an engineer I can move a lot faster and more easily dynamically update the
map. However, I had a really difficult time getting the map to render - for some reason, only half
of the tiles on the map would load, leaving the map a mix of black squares and fully filled map
tiles.

If this were a real product, I'd spend some time digging into why this was happening, but since I
was on a timeline I ended up switching over to an imperatively rendered leaflet map. Currently, I'm
completely tearing down and reinitializing the map whenever any data on the map changes, and that's
obviously not ideal. To implement some of the more advanced future ideas I mentioned below (in
particular, the hover behavior), I'd have to make some significant changes to how this works -
either dynamically add/modify/remove entities on the map through a d3 data join like process, or I'd
need to get something like react-leaflet working.
...
units of `eastings` and `northings`. Not what I was expecting!
...
`xxx.prj` file for the `xxx.shp` file in the zip, fed all the raw x/y values into `proj4` converting
to `WGS84`, and stored the resulting latitudes and longitudes into the application state.

### Map centering
After getting the shapefile parsing logic wired up, I uploaded a shapefile, and... I couldn't see it
on the map? But, panning around a bit, I noticed that it was on the other side of the world! This
was obviously a poor user experience - what somebody would expect in this case is that after
uploading a file, the map's location would change to be centered on the shapefile geometry.

For geojson like arithmetic operations like this, I've found [turf.js](https://turfjs.org/) to be
excellent in the past. So, I ended up computing a center point for each subfile, and then combining
all these centers together to get a center point for the whole shapefile, and passed that into the
`center` parameter when constructing the leaflet map. I didn't have time, but ideally, the leaflet
map's zoom level would also be updated so that the bounding box of the shapefile was contained
...
within a multipolygon as one, combined polygon rather than the entire multipolygon as one entity? I
didn't have the time to dig into this but obviously this might be a big problem for more complex
shapefiles.

<img src="/readme3.png" width="25%" />


## Future Improvements
As I was building this application, I focused on getting something out the door that worked ASAP.
With more time, here's a list in no particular order of what I'd focus on depending on the business
...
on the backend:
1. Upload the shapefile to object storage (s3, etc)
2. Make a request to a server, specifying the uploaded shapefile
3. When handling that request, the server puts a message in a queue / kafka topic / etc, and
   responds with a 202
...
arise that could be hard to debug.

Doing this serverside is definitely not where I'd start but it's something to keep in mind as the
application grows.
...
to show and hide subfiles, maybe by adding a checkbox in front of each one, so a user can hide data
...
of where each subfile is located on the map.
...
other geometry types with more time. Right now, non polygons are just skipped while rendering.

### General performance improvements
For larger shapefiles, I've noticed that the map is somewhat slow, likely because I'm rendering so
many individual polygons. I suspect that if I combined a lot of these polygons together, maybe using
[`turf.union`](https://turfjs.org/docs/api/union) or something similar and then rendered THOSE as
maybe multipolygons, that could be the way to go. I didn't get a chance to explore this due to time,
however!
...
- Some sort of linting, like `eslint`: Again, a similar rationale - on a team, these tools can be
  ...
  next person. This next app does contain some default linting configuration, but I haven't run it
  or reformatted all my code to match those rules. This should be done before any more significant
  work happens on the app!
...
### General refactoring
I spent most of my ~3 hours building actual functionality, but there's a lot that could be done to
